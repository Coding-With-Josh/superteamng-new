...
export const HeroSection = () => {
  return (
    <section className="relative mx-2 mt-1 mb-4 flex min-h-[calc(100vh-5.5rem)] w-full items-center justify-center overflow-hidden rounded-xl py-6 sm:mx-4 sm:py-10">
      ...
      >
        <span className="select-none font-bold text-[7rem] text-black/3 tracking-tighter sm:text-[8rem] md:text-[10rem] lg:text-[10rem] dark:text-white/3">
          SuperteamNG
        </span>
      </div>
      <div
        ...
        <div className="space-y-8 text-center">
          <div className="flex justify-center">
            <Link
              ...
              className="inline-flex items-center gap-2 text-black/70 text-md tracking-tighter dark:text-white/70 bg-green-500/10 rounded-full px-4 py-1 border border-emerald-600/70 hover:bg-green-500/15 hover:scale-105 transition-all duration-300"
            >
              <Icon icon="simple-icons:x" className="w-3 h-3" />
              <span className="text-sm">We just crossed $1M in GDP</span>
              <ArrowUpRightIcon className="w-4 h-4 ml-2" />
            </Link>
          </div>

          <h1 className="font-semibold text-4xl tracking-tight font-serif sm:text-5xl md:text-6xl lg:text-7xl">
            A Community. Built for You <br />
            Built,{" "}
            <span className="italic text-emerald-500/85 dark:text-emerald-500/85">
              for ALL.
            </span>
          </h1>
            ...
            <div className="flex items-center gap-2">
              <div className="-space-x-3 flex">
                {[1, 2, 3, 4].map((i) => (
                  <div
                    key={i}
                    className="relative h-9 w-9 overflow-hidden rounded-full border-2 border-white shadow-md transition-all hover:z-10 hover:scale-110 dark:border-black"
                  >
                    <Image
                      src={`/community/${i}.png`}
                      alt={`Member ${i}`}
